...
Skopiuj folder z projektem na dysk lokalny.
### 3.2 Otwarcie projektu: 
Uruchom plik Bartosz_Lacny_projekt_bazy_danych.sln w Visual Studio.
### 3.3 Przywracanie pakietów: 
Visual Studio automatycznie pobierze brakujące biblioteki NuGet przy pierwszym zbudowaniu.
### 3.4 Tworzenie bazy danych:
- Wykonaj punkt 4.1 w celu zmiany nazwy serwera
...
Naciśnij klawisz F5 lub zieloną strzałkę "Start" w Visual Studio.

---


## 4. Konfiguracja
### 4.1 Połączenie z bazą
...
```
_w przypadku uruchamiania na innym komputerze, należy edytować fragment `Server` i zmienić go na swój własny_

### 4.2 Użytkownicy Testowi i Hasła
...
- __Login:__ `admin@schronisko.pl`
- __Hasło:__ `Haslo123!`
...
### 5.1 Użytkownik Niezalogowany (Gość)
...
### 5.2 Zalogowany Użytkownik (Rola: User)
...
### 6.1 Wzorzec MVC: 
Aplikacja podzielona na Modele, Widoki i Kontrolery

### 6.2 Encje: 
Projekt zwaiera modele połączone relacjami:
- Animal (Encja główna)
...
### 6.3 API: 
Zaimplementowano API typu REST dla głównej encji (Zwierzęta):
